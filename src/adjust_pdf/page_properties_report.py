...
def _format_page_line(page_number: int, rotation: int, width: float, height: float) -> str:
    """生成单页属性行，有旋转时包裹红色标记。"""
    line = f"第 {page_number} 页：旋转角度：{rotation}°，{width:.2f}（宽） × {height:.2f}（高）"
    if rotation != 0:
...
def format_page_property_detail(info: DocumentInfo) -> str:
    ...
    for page in info.pages:
        lines.append(_format_page_line(page.page_number, page.rotation, page.width, page.height))
...
def format_multiple_page_property_reports(infos: list[DocumentInfo]) -> str:
    ...
    separator = escape("\n\n" + "=" * 70 + "\n\n")
    return separator.join(format_page_property_detail(info) for info in infos)
