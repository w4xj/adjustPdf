...
本文说明 PDF·如一 的 LTY 背景皮肤、PySide6 界面结构、资源生成方式和后续扩展边界。皮肤只属于图形界面，不参与 PDF 读取、旋转消除、签名检测或文件写出。
...
卡片使用高透明深蓝玻璃底（约 20% 不透明度）、暖金细边框和轻阴影。卡片四周、卡片之间，以及表格/输入框/按钮内部都应能看到原图细节，而不是只露出窗口边缘。
...
- 输入框、表格和按钮使用高透明深色玻璃底 + 浅色加粗文字，优先透出原图，同时靠边框和对比度保证可读。
...
初版已经实现全局背景和半透明卡片，但卡片与表格仍偏实，原图在控件内部不够通透。

### 10.2 0.5.2 高透明验收结果

0.5.2 将卡片、表格、输入框和按钮进一步改为高透明玻璃风格，并显著降低背景压暗层：

![PySide6 LTY 高透明皮肤效果](../screenshot/pyside6-0.5.2-exe-final.png)

正式 EXE 实机截图验收数据（含窗口边框 1116×829）：

- 左侧全局留白与原图相关系数约 `1.000`；
- 标题与文件卡片间隔约 `0.977`；
- 文件列表卡片中心约 `0.818`；
- 处理选项卡片中心约 `0.883`；
- 执行状态卡片中心约 `0.876`；
- 红色警示采样约 `RGB(178, 99, 109)`，保持红色警示且不完全遮死背景。

离屏渲染下文件列表中心相关系数可到约 `0.925`，说明高透明样式本身能通透展示原图；实机截图会受 Windows 窗口合成影响，但仍明显可见壁纸。

验收标准：在不牺牲按钮边框和功能文字可读性的前提下，界面应几乎能看到 LTY 原图。

### 10.3 历史 EXE 截图说明

正式构建的 `dist\PDF-如一.exe` 已于 2026 年 7 月 24 日实际启动并自动截取主窗口：

![PySide6 LTY 皮肤最终效果](../screenshot/pyside6-0.5.1-exe-final.png)

验收数据：

- EXE 窗口标题：`PDF·如一 0.5.3`；
