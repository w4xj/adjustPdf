...
- GUI 已重构为 PySide6，使用 `assets/lty.jpg` 提供全窗口 LTY 背景；功能区采用半透明悬浮卡片，卡片内部和间隔均可看到壁纸；
- 电子签章风险使用红色常驻警示条显示。

## 界面预览

下图由正式构建的 0.5.1 EXE 实际启动后截取，展示 PySide6 全局背景、半透明悬浮卡片和红色签章风险提示：
...
程序会检测标准 PDF 数字签名结构。图形界面检测到签名时会先弹窗确认，默认取消；命令行模式默认拒绝处理已签名 PDF，需要明确添加 `--allow-signed`。继续处理会使新生成文件的签名验证失效，原文件不会被覆盖。
